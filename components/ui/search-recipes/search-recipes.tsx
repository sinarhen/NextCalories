...
const SearchRecipes = () => {
  ...
  const {makeParameters, getParameter} = useQueryParams();
  ...
  useEffect(() => {
    ...
    if (mealType) setMealTypeFilter(mealType);
    if (cuisineType) setCuisineTypeFilter(cuisineType);
    if (diet) setDietTypeFilter(diet);
    if (health) setHealthFilter(health);
    if (dishType) setDishTypeFilter(dishType);
  ...
  const onSubmit = useCallback(() => {
    try {
      ...
      router.push(`/recipes${params ? '?' + params : ''}`, {scroll: false});
    } catch (err) {
  ...
  useEffect(() => {
    const randomSuggestedItems = getRandomRecipesNames(4);
    setSuggestedItems(randomSuggestedItems);
  }, []);

  // Background image style
  const divStyle: React.CSSProperties = {
    backgroundImage: `url("https://images.pexels.com/photos/326281/pexels-photo-326281.jpeg?cs=srgb&dl=pexels-pixabay-326281.jpg&fm=jpg")`,
  };

  return (
    <div style={divStyle} className={twMerge('flex w-full bg-cover bg-center items-center space-y-8')}>
      <div
        className="w-full text-lg md:text-sm text-white h-full bg-black/60 md:py-40 sm:py-30 py-16 flex items-center">
        <div className="md:px-48 lg:px-72 px-14 h-full w-full flex flex-col items-center">
          <h1 className="font-bold text-5xl md:text-4xl">
            {query ? `Search "${query}"` : 'Search Recipes '}
          </h1>
